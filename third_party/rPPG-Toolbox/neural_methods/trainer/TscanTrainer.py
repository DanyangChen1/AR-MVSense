...
class TscanTrainer(BaseTrainer):
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            ...
            if resume_path:
                if not os.path.exists(resume_path):
                    raise ValueError("Resume model path does not exist: {}".format(resume_path))
                epoch_match = re.search(r'_Epoch(\d+)\.pth$', resume_path)
                if epoch_match is None:
                    raise ValueError("Resume model path must end with _Epoch<N>.pth: {}".format(resume_path))
                resume_epoch = int(epoch_match.group(1))
                self.start_epoch = resume_epoch + 1
                self.model.load_state_dict(torch.load(resume_path, map_location=self.device, weights_only=True))
                metadata_path = resume_path + '.resume.json'
                if os.path.exists(metadata_path):
                    with open(metadata_path, 'r') as metadata_file:
                        metadata = json.load(metadata_file)
                    self.best_epoch = int(metadata['best_epoch'])
                    self.min_valid_loss = float(metadata['min_valid_loss'])
                self.resume_mode = 'weights_only'
                print("Resuming TSCAN weights from epoch {} (optimizer/scheduler reset).".format(resume_epoch))
            # See more details on the OneCycleLR scheduler here: https://pytorch.org/docs/stable/generated/torch.optim.lr_scheduler.OneCycleLR.html
            remaining_epochs = max(1, config.TRAIN.EPOCHS - self.start_epoch)
            self.scheduler = torch.optim.lr_scheduler.OneCycleLR(
                self.optimizer, max_lr=config.TRAIN.LR, epochs=remaining_epochs, steps_per_epoch=self.num_train_batches)
            if resume_path:
                trainer_state_path = resume_path + '.trainer.pth'
                if os.path.exists(trainer_state_path):
                    trainer_state = torch.load(trainer_state_path, map_location=self.device, weights_only=False)
                    ...
                    self.min_valid_loss = float(trainer_state['min_valid_loss'])
                    self.resume_mode = 'exact_trainer_state'
                    print("Restored optimizer and scheduler state from {}.".format(trainer_state_path))
    ...
    def save_trainer_state(self, index):
        """Save restart state separately so model checkpoints stay Toolbox-compatible."""
        model_path = os.path.join(
            self.model_dir, self.model_file_name + '_Epoch' + str(index) + '.pth')
        torch.save({
            'epoch': index,
        ...
        }, model_path + '.trainer.pth')
        with open(model_path + '.resume.json', 'w') as metadata_file:
            json.dump({
                'epoch': index,
                'best_epoch': self.best_epoch,
                'min_valid_loss': self.min_valid_loss,
